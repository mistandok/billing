...
def create_customer_subscribe_payment_link(user_id: int, email: str, subscribe_type: str) -> str:
    """
    ...
    return payment_system.create_checkout_session_url(consumer=customer, subscribe=subscribe)


def cancel_subscribe_for_user(user_id: int, email: str, subscribe_type: str) -> bool:
    """
    ...
    """
    customer = Consumer.objects.filter(user_id=user_id, email=email, subscribe__subscribe_type=subscribe_type).first()
    if customer:
        subscribe = Subscribe.objects.get(subscribe_type=subscribe_type)
        payment_system = get_payment_system_by_name(PaymentSystemName.STRIPE)
        payment_system.cancel_subscribe(customer, subscribe)
        return True
